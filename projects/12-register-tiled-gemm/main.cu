// project 12 - Register Tiled GEMM - main.cu
...
static constexpr int REGISTER = 2;
...
// GPU REGISTER TILED
__global__ void gemmRegisterTiled(
    const float* __restrict__ A,
    const float* __restrict__ B,
    float* __restrict__ C,
    int M, int N, int K)
{
    ...
    const int row0 = (blockIdx.y * BLOCK_SIZE) + ty * REGISTER;
    const int col0 = (blockIdx.x * BLOCK_SIZE) + tx * REGISTER;

    float acc[REGISTER][REGISTER] = {0.0f};
    ...
    for (int t = 0; t < tiles; ++t) {
        ...
        for (int i = 0; i < REGISTER; ++i) {         
            const int aRow = row0 + i;
            #pragma unroll
            for (int j = 0; j < REGISTER; ++j) {     
                const int aCol = t * BLOCK_SIZE + tx * REGISTER + j;
                AShared[ty * REGISTER + i][tx * REGISTER + j] =
        ...
        #pragma unroll
        for (int i = 0; i < REGISTER; ++i) {          
            const int bRow = t * BLOCK_SIZE + ty * REGISTER + i;
            #pragma unroll
            for (int j = 0; j < REGISTER; ++j) {    
                const int bCol = col0 + j;
                BShared[ty * REGISTER + i][tx * REGISTER + j] =
        ...
        for (int k = 0; k < BLOCK_SIZE; ++k) {
            ...
            #pragma unroll
            for (int i = 0; i < REGISTER; ++i)
                aReg[i] = AShared[ty * REGISTER + i][k];

            #pragma unroll
            for (int j = 0; j < REGISTER; ++j)
                bReg[j] = BShared[k][tx * REGISTER + j];
...
int main() {
    ...
    dim3 blockReg(BLOCK_SIZE/REGISTER, BLOCK_SIZE/REGISTER);
    dim3 gridReg((N + BLOCK_SIZE - 1)/BLOCK_SIZE, (M + BLOCK_SIZE - 1)/BLOCK_SIZE);
    ...
    CUDA_OK(cudaMemset(dC, 0, sizeC));
    CUDA_OK(cudaEventRecord(start));
    gemmRegisterTiled<<<gridReg, blockReg>>>(dA, dB, dC, M, N, K);
    CUDA_OK(cudaGetLastError());
    CUDA_OK(cudaEventRecord(end));
    CUDA_OK(cudaEventSynchronize(end));

    float msReg = 0.0f;
    CUDA_OK(cudaEventElapsedTime(&msReg, start, end));
    CUDA_OK(cudaMemcpy(hC.data(), dC, sizeC, cudaMemcpyDeviceToHost));

    double maxDiffReg = 0.0;
    for (int i = 0; i < M * N; ++i) {
        double diff = std::abs((double)hC[i] - (double)hRef[i]);
        maxDiffReg = std::max(maxDiffReg, diff);
    }
    {
        const double flops = 2.0 * M * N * K;
        const double gflops = flops / (msReg / 1000.0) / 1e9;   
        printf("GPU Register Tiled: %.3f ms, %.1f GFLOP/s\n", msReg, gflops); 
        printf("Max diff: %.6f\n", maxDiffReg);              
    }
